...
def test_unpriced_sessions_skipped():
    ...
    ledger = compute_cost_ledger(
        [priced, unpriced_baseline, unpriced_weekly],
        as_of=AS_OF,
    )
    assert ledger.baseline_session_count == 1
    assert ledger.baseline_weekly_mean_usd == 3.0
...
def test_biggest_line_current_week_boundary_is_monday_inclusive():
    ...
    monday_session = BiggestLineInputSession(
        started_at=datetime(2026, 5, 25, 0, 0, tzinfo=UTC),  # Mon 00:00
        cost_usd=4.0,
        model_hint="opus",
        task_label="A",
    )
...
def test_tier_fit_single_qualifying_session_counted():
    ...
    s = _tierfit_session(
        days_ago=0, user_turn_count=2, avg_prompt_chars=100.0, tier_fit_savings_usd=1.25
    )
...
def test_tier_fit_skips_sessions_with_too_many_turns():
    ...
    s = _tierfit_session(
        days_ago=0, user_turn_count=4, avg_prompt_chars=50.0, tier_fit_savings_usd=1.0
    )
    out = compute_tier_fit_savings([s], as_of=AS_OF)
    assert out.qualifying_session_count == 0
    assert out.estimated_savings_usd == 0.0
...
def test_tier_fit_skips_sessions_with_long_prompts():
    ...
    s = _tierfit_session(
        days_ago=0, user_turn_count=1, avg_prompt_chars=201.0, tier_fit_savings_usd=1.0
    )
    out = compute_tier_fit_savings([s], as_of=AS_OF)
    assert out.qualifying_session_count == 0
...
def test_tier_fit_both_thresholds_must_be_met():
    ...
    long_prompts = _tierfit_session(
        days_ago=0, user_turn_count=1, avg_prompt_chars=500.0, tier_fit_savings_usd=1.0
    )
    many_turns = _tierfit_session(
        days_ago=1, user_turn_count=10, avg_prompt_chars=50.0, tier_fit_savings_usd=1.0
    )
...
def test_tier_fit_thresholds_are_inclusive():
    ...
    edge_turns = _tierfit_session(
        days_ago=0,
        user_turn_count=TIER_FIT_MAX_USER_TURNS,
        avg_prompt_chars=50.0,
        tier_fit_savings_usd=0.5,
    )
    edge_chars = _tierfit_session(
        days_ago=1,
        user_turn_count=1,
...
def test_tier_fit_zero_savings_still_counts_as_qualifying():
    ...
    """
    s = _tierfit_session(
        days_ago=0, user_turn_count=1, avg_prompt_chars=0.0, tier_fit_savings_usd=0.0
    )
    out = compute_tier_fit_savings([s], as_of=AS_OF)
    assert out.qualifying_session_count == 1
    assert out.estimated_savings_usd == 0.0
...
def test_moment_impact_fit_uses_session_total_input_chars(tmp_home):
    ...
    got = compute_moment_dollar_impact_usd(
        "fit",
        "claude-opus-4-7",
        session_total_input_chars=4000,
    )
    assert expected is not None and got is not None
    assert abs(got - expected) < 1e-9
