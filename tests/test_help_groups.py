...
def test_loop_stubs_exit_zero(capsys):
    """commit/nudge/reflect/install-coach print a friendly message and exit 0."""
    for verb in ("commit", "nudge", "reflect", "install-coach"):
        code = main([verb])
        out = capsys.readouterr().out
        assert code == 0, f"`praxis {verb}` stub should exit 0, got {code}"
        assert verb in out, (
            f"`praxis {verb}` stub output should name the verb so the user "
            f"knows which command is not wired up yet."
        )
